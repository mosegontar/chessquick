function share()  {
    prompt("Share your game with this link!", root_path + match_url);
};
...
function update(action, match_url, current_player, game) {
    if (game === undefined) {
        game = true;
    }
    ...
    $.getJSON($SCRIPT_ROOT +'/_save', {
        ...
        if (data.resp == 'need confirmation') {
            window.location.assign(root_path+match_url);
        } else {
            toggled();
...
function showSave() {
    ...
    document.getElementById("save_link").onclick = function (){ 
        update('save', match_url, current_player);
    };
    update('unnotify', match_url, current_player);
    $('#notify_link').hide();
    $('#notify_span').hide();
};
...
function showUnsave() {
    ...
    $('#save_span').show();    
    document.getElementById("save_link").onclick = function () {
        update('unsave', match_url, current_player);
    };
    $('#notify_link').show();  
    $('#notify_span').show();
};
...
function notifyOff() {
    ...
    document.getElementById('notify_link').onclick = function () {
        update('notify', match_url, current_player);
    };
};
